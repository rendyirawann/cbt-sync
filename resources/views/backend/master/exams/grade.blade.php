...
@section('content')
@php $isManual = $exam->points_mode === 'manual'; @endphp
                    ...
                    @else
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" step="0.01" min="0" max="{{ $weights[$q->id] }}" name="scores[{{ $q->id }}]" class="form-control" value="{{ $ans && $ans->earned_score !== null ? rtrim(rtrim((string)$ans->earned_score,'0'),'.') : '' }}">
                            </div>
                            <div class="col-md-8 mb-3">
                                <label class="form-label">Catatan / Feedback</label>
                                <input type="text" name="feedback[{{ $q->id }}]" class="form-control" value="{{ $ans->feedback ?? '' }}">
                            </div>
                        </div>
                    @endif
                </div>
            ...
            <div class="card mb-6">
                <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" name="final_score" class="form-control w-200px" value="{{ $attempt->final_score !== null ? rtrim(rtrim((string)$attempt->final_score,'0'),'.') : '' }}" required>
                    </div>
                    @else
                    ...
                    @endif
                    <button type="submit" class="btn btn-primary"><i class="ki-outline ki-check fs-4"></i> Simpan Nilai</button>
                </div>
            </div>
        </form>
    </div>
